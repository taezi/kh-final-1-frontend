...
export default function RestViewPage() {
  ...
  }, [restNo]); // cafeNo가 변경될 때도 스크롤하도록 추가 // 컴포넌트가 처음 렌더링되거나 URL 파라미터가 변경될 때마다 API를 호출합니다.
  ...
  const imgs = useMemo(() => {
    const thumbUrl =
      rest?.restImgAddress || "https://picsum.photos/1200/800?blur=2";
    return [thumbUrl];
  }, [rest]); // 로딩 중일 때 보여줄 화면
  ...
  return (
    <div className="page cv-wrap">
      ...
      <h1 className="cv-title center">
        {rest.restName} {rest.restBranch}
        <button
          className={`hearts editor-detail-hearts ${
            likes.has(rest.restNo) ? "is-on" : ""
          }`}
          onClick={(e) => {
