...
export default function CafeViewPage() {
  ...
  const [likes, setLikes] = useState(() => new Set()); // 상태 관리: 데이터를 담을 공간, 로딩 상태, 에러 메시지
  const [cafe, setCafe] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(""); // 페이지가 로딩될 때마다 맨 위로 스크롤합니다.
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: "instant" });
  }, [cafeNo]); // cafeNo가 변경될 때도 스크롤하도록 추가 // 컴포넌트가 처음 렌더링되거나 URL 파라미터가 변경될 때마다 API를 호출합니다.
  ...
  // 댓글 조회
  useEffect(() => {
    if (cafeNo) fetchComments("cafe", cafeNo);
  }, [cafeNo]);
  ...
  const handleAddComment = async () => {
    ...
    try {
      const saved = await addComment("cafe", cafe.cafeNo, newComment);
  ...
  const handleDeleteComment = async (reviewno) => {
    ...
    try {
      await removeComment(reviewno, "cafe");
  ...
  const imgs = useMemo(() => {
    const thumbUrl =
      cafe?.cafeImgAddress || "https://picsum.photos/1200/800?blur=2";
    return [thumbUrl];
  }, [cafe]); // 로딩 중일 때 보여줄 화면
  ...
  return (
    <div className="page cv-wrap">
      ...
      <div className="cv-topline center">
               {" "}
        {cafe.cafeName && <span className="cv-chip">{cafe.cafeName}</span>}     
         {" "}
        {cafe.cafeBranch && <span className="cv-chip">{cafe.cafeBranch}</span>} 
           {" "}
      </div>
           {" "}
      <h1 className="cv-title center">
        {cafe.cafeName} {cafe.cafeBranch}
        <button
          className={`hearts editor-detail-hearts ${
            likes.has(cafe.cafeNo) ? "is-on" : ""
          }`}
          onClick={(e) => {
            e.stopPropagation(); // 카드 클릭 이벤트와 겹치지 않도록
            toggleLike(cafe.cafeNo, e); // data.cafeNo를 인자로 전달
          }}
          title="좋아요"
        >
                  ♡      {" "}
      ...
      <div className="cv-heroimg">
                <img src={imgs[0]} alt={cafe.cafeName} />     {" "}
      </div>
           {" "}
      <section className="cv-card">
               {" "}
        <dl className="cv-dl">
                    <InfoItem label="주소" value={cafe.cafeAddress} />
                    <InfoItem label="전화번호" value={cafe.cafePhonNumber} />
                    <InfoItem label="종류" value={cafe.cafeType} />
                    <InfoItem label="영업 시간" value={cafe.cafeOpen} />
                    <InfoItem label="지역" value={cafe.cafeRegion} />
                    <InfoItem label="평점" value={cafe.cafeRating} />         {" "}
          {/* 웹사이트 URL (링크로 표시) */}         {" "}
          {cafe.cafeWebsite && (
            <>
                            <dt>웹사이트</dt>             {" "}
